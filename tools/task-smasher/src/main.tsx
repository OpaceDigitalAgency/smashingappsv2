...
// Create routes for each use case - handle with and without trailing slashes
const useCaseRoutes = Object.entries(useCaseDefinitions).flatMap(([id, definition]) => {
  const basePath = `/tools/task-smasher/${definition.label.toLowerCase().replace(/\s+/g, '-')}`;
  const pathWithSlash = `${basePath}/`;
  
  // Common element to render for both routes
  const routeElement = (
    <>
      <Helmet>
  ...
  // Return two Route components: one for path without trailing slash, one with
  return [
    ...
    <HelmetProvider>
      <ReCaptchaProvider>
        <TasksProvider>
          <BrowserRouter>
            <Routes>
              {/* Root path for Task Smasher - both with and without trailing slash */}
