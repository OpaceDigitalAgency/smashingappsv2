...
const MenuBar: React.FC = () => {
  ...
  const menus: MenuSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Modify', submenu: [
          { label: 'Border...', action: wrapHandler(() => menuHandlers.modifySelection('border', activeDocumentId)), disabled: !selection },
          { label: 'Smooth...', action: wrapHandler(() => menuHandlers.modifySelection('smooth', activeDocumentId)), disabled: !selection },
          { label: 'Expand...', action: wrapHandler(() => menuHandlers.modifySelection('expand', activeDocumentId)), disabled: !selection },
          { label: 'Contract...', action: wrapHandler(() => menuHandlers.modifySelection('contract', activeDocumentId)), disabled: !selection },
          { label: 'Feather...', shortcut: '⇧F6', action: wrapHandler(() => menuHandlers.modifySelection('feather', activeDocumentId)), disabled: !selection },
        ]},
        { label: 'Grow', action: wrapHandler(() => menuHandlers.growSelection(activeDocumentId)), disabled: !selection },
        { label: 'Similar', action: wrapHandler(() => menuHandlers.similarSelection(activeDocumentId)), disabled: !selection },
        { divider: true },
        { label: 'From Layer Alpha', action: wrapHandler(() => menuHandlers.fromLayerAlpha(activeDocumentId)), disabled: !activeDocument?.activeLayerId },
        { divider: true },
        { label: 'Transform Selection', action: wrapHandler(() => menuHandlers.transformSelection(activeDocumentId)), disabled: !selection },
        { label: 'Save Selection...', action: wrapHandler(() => menuHandlers.saveSelection(activeDocumentId)), disabled: !selection },
        { label: 'Load Selection...', action: wrapHandler(() => menuHandlers.loadSelection()), disabled: !activeDocumentId },
      ]
    },
    {
      title: 'Filter',
    ...
    },
    {
      title: 'Window',
      items: [
        { label: 'Toggle Layers', shortcut: 'F7', action: wrapHandler(() => menuHandlers.togglePanel('layers')) },
        { label: 'Toggle History', shortcut: 'F9', action: wrapHandler(() => menuHandlers.togglePanel('history')) },
        { label: 'Toggle Properties', action: wrapHandler(() => menuHandlers.togglePanel('properties')) },
        { label: 'Toggle Adjustments', action: wrapHandler(() => menuHandlers.togglePanel('adjustments')) },
        { divider: true },
