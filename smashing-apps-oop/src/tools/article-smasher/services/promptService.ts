...
const DEFAULT_PROMPTS: PromptTemplate[] = [
  ...
  },
  {
    ...
    systemPrompt: 'You are a professional SEO content writer. Your task is to write a complete, detailed article based on the topic and keywords provided. Write full paragraphs with proper prose, not just an outline or bullet points.',
    userPromptTemplate: 'Write a full article titled "{{title}}" following this outline: {{outline}}. Naturally incorporate these keywords: {{keywords}}. Write complete paragraphs with detailed explanations for each section. The article should be engaging, informative, and written in a conversational tone. Use proper headings (# for H1, ## for H2, etc.) for each section, but write full prose content under each heading, not bullet points.',
    category: 'content',
    temperature: 0.7,
    maxTokens: 4000,
    createdAt: new Date(),
